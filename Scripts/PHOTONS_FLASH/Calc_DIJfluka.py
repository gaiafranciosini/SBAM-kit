...
# ==============================================================================
# MAIN PROGRAM: CONFRONTO E SALVATAGGIO IN FORMATO BINARIO DIJ
# ==============================================================================
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python script.py Dijopt.bin Dijfluka.bin fieldID metodo")
        print("Metodi disponibili:")
        print("  1 : Minimi Quadrati (Linear Least Squares Voxel-by-Voxel)")
        print("  2 : Dose Integrale (Rapporto delle somme volumetriche)")
        sys.exit(1)

    file_opt = sys.argv[1]
    file_fluka = sys.argv[2]
    FID = int(sys.argv[3])
    metodo = int(sys.argv[4])

    if metodo not in [1, 2]:
        print("Errore: Il metodo deve essere 1 o 2.")
        sys.exit(-1)

    mu = 0.2  # Coefficiente di attenuazione del Tungsteno (W)
    soglia_percentuale = 0.05  # Soglia al 5% del max per isolare il fascio

    nome_metodo = (
        "Minimi Quadrati (Metodo 1)"
        if metodo == 1
        else "Dose Integrale (Metodo 2)"
    )
    print(f"--- Configurazione: Utilizzo {nome_metodo} ---")
    print("Leggo OPT...")
    pb_opt, _ = read_all_pencilbeams(file_opt, FID)
    print("Leggo FLUKA...")
    pb_fluka, header_originale = read_all_pencilbeams(file_fluka, FID)
    ...
    # Definiamo il nome del nuovo file Dij binario in uscita
    suffisso = "LS" if metodo == 1 else "Integral"
    out_binary_fname = f"Dijfluka_riscalata_{suffisso}_Field_{FID}.bin"
    ...
    print(f"\n--- Calcolo spessori e scrittura Dij raggio per raggio ---")
    print(f"{'PB (j)':<10}{'Voxel Usati':<15}{'Delta_x (W)':<15}")
    print("-" * 45)
    ...
    for j in sorted(pids_comuni):
        ...
        # Soglia sul file OPT per eliminare il background
        soglia = soglia_percentuale * np.max(M_opt_j)

        # Maschera per il calcolo dello spessore
        mask_calc = (M_opt_j > soglia) & (M_fluka_j > 0)
        num_voxel_validi = np.sum(mask_calc)

        delta_x_j = 0.0

        if num_voxel_validi >= 5:
            # Metodo 1: Minimi Quadrati
            if metodo == 1:
                d_opt_validi = M_opt_j[mask_calc]
                d_fluka_validi = M_fluka_j[mask_calc]
                Y = np.log(d_opt_validi / d_fluka_validi)
                delta_x_j = -(1.0 / mu) * np.mean(Y)

            # Metodo 2: Dose Integrale
            elif metodo == 2:
                somma_opt_j = np.sum(M_opt_j[mask_calc])
                somma_fluka_j = np.sum(M_fluka_j[mask_calc])
                if somma_fluka_j > 1e-6 and somma_opt_j > 1e-6:
                    rapporto_integrale = somma_opt_j / somma_fluka_j
                    delta_x_j = -(1.0 / mu) * np.log(rapporto_integrale)

        print(f"{j:<10}{num_voxel_validi:<15}{delta_x_j:<15.4f}")
        ...
        # Selezioniamo solo i voxel attivi (dose > 0) per preservare la compressione
        indici_attivi = np.where(M_flat > 0)[0].astype("uint32")
        valori_attivi = M_flat[indici_attivi].astype("float32")
        numvxl = len(indici_attivi)

        # 2. Scrittura del blocco nel nuovo file binario:
        # Intestazione del blocco (pid, fid, numvxl) -> 3 interi (12 byte)
        fout.write(struct.pack("3i", j, FID, numvxl))
    ...
    fout.close()
    print("\n" + "=" * 45)
    print(f"Nuovo file Dij binario salvato con successo!")
    print(f"File generato: {out_binary_fname}")
