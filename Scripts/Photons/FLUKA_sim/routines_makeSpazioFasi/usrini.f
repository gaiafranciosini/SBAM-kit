...
c
      idbflg = int(what(1))
      iwritesel = int(what(2))
      ifragflag = int(what(3))
      ibplusflag = int(what(4))
c 
...
c
      ...
      do ii = 1,NREGS
         ...
         if(ierr.eq.0) then
            if(REGNAM(1:3).eq.'VAC') then
               nregvuoto = ii
            elseif(REGNAM(1:7).eq.'Phantom') then
               nregtriumf = ii
            elseif(REGNAM(1:3).eq.'OUT') then
               nregblk = ii
            endif
         endif
      end do
      ...
      write(*,*) ' '
      write(*,*) ' nregvuoto =  ',nregvuoto,' nregtriumf= ',nregtriumf
      write(*,*) ' nregblk =  ',nregblk
c    
      nregcounter=nregvuoto*nregtriumf*nregblk
