...
template<typename T>
void write_mhd(string fname,T *A,int nn[3],float x0[3],float L[3],float left[3],float up[3],float front[3],bool localData=true){
	string dtype = dataTypeString<T>();
	write_mhd(fname,dtype,(char *)A,nn,x0,L,left,up,front,localData);
}
