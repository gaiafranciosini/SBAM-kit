...
public:
	...
	vector<float64> wei; // voxel weights
        float64 plannedDose,maxDose,alphabeta,Dthr;
	float FMFmin;
...
class Optimizer{
	...
	float FMFmin_NT = 0.6;     // DMF for Normal Tissue
	...
	int Reduction_NT = 1;   // reduction factor for NT
        float alphabeta_NT = 3.0;
        float Dthr_NT=6;
 

	float FMFmin_DCTRL = 1.0;     // DMF for Dose Control Volume
	...
	float Reduction_DCTRL = 1.0;   // reduction factor for Dose Control Volume
        float alphabeta_DCTRL = 3.0;
        float Dthr_DCTRL=6.0;
  

        static int numFractions;


	vector<float> FMFmin; // dense DMF map
        vector<float> Dthr;
	...
	vector<int> Dij_npb;
        ...
        vector<float64> Dgoal;
        vector<vector<float64>> D;
        vector<float> alphabeta;
	...
	Optimizer() {
		conv = 0.0005;
		fScaleOptimizationStep = 0.01;//1.0;
		itermax = 10000;
		numVxl=npb=-1;// not set!!!
		numVxlAll=-1;
		numVxlPTV=numVxlOAR=-1;
		f_flatOptimizer = true;
		f_Debug = false;
		pthreads_max_num=6;
	};
	...
	float64 getPBPrimaryNum(int jpb);
        ...
        static void setNumFractions(int numFractions);
  ...
  //	void filterDij_bin(string fpath, int call);
	...
	void init_DMF();
        double compute_FMF(double d, int vxl);

	void loadROI_mhd(ROI &roi,string fpath);
        void loadPTV_mhd(string ID,string fpath,float plannedDose,float weight, float FMFmin, float alphabeta,float Dthr);
        void loadOAR_mhd(string ID,string fpath,float maxDose,float weight, float FMFmin, float alphabeta,float Dthr);
